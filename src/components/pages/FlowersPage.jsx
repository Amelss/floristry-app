...
export default function FlowersPage() {
  ...
  const visible = FLOWERS.filter((f) => {
    ...
    const mF =
      filter === "all" ||
      (isRole && f.roles.includes(filter.replace("role-", ""))) ||
      (!isRole && f.tags.includes(filter));
    const mS =
      !search ||
    ...
    <div>
      <div className="bg-[#3D5C3A] px-5 sm:px-14 py-10 sm:py-12 relative overflow-hidden">
        ...
        <p className="text-[10px] tracking-[0.22em] uppercase text-[#B8CEAE] mb-4 font-medium">
          Free Online Learning Plan · UK Edition
        </p>
        <h1
        ...
        <p className="text-[13px] text-white/55 font-light max-w-md leading-relaxed mb-6 sm:mb-8">
          Everything a floristry student needs to know. Common names, Latin
      ...
      <InfoBand
        items={[
          [
            "Seasonality bars",
            "Green blocks show the months each flower is naturally available from UK growers.",
          ],
          [
            "Import note",
            "Many flowers are available via import year-round even outside their UK season.",
          ],
          [
            "Flower roles",
      ...
      <div className="md:hidden px-4 py-3 border-b border-stone-100 bg-white">
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-none">
          {[
            ...
            { key: "role-Focal", label: "Focal" },
            { key: "role-Secondary", label: "Secondary" },
            { key: "role-Filler", label: "Filler" },
            { key: "role-Line", label: "Line" },
            { key: "role-Foliage", label: "Foliage" },
            { key: "role-Texture", label: "Texture" },
          ].map((f) => (
            <button
              key={f.key}
