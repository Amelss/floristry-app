    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
      {Object.entries(ROLE_CONFIG).map(([role, cfg]) => (
        <div key={role} className="flex items-center gap-2.5">
          <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{background: cfg.colour}}/>
          <span className="text-[11px] text-stone-600 font-medium w-20 flex-shrink-0">{cfg.label}</span>
          <div className="flex items-center gap-1.5">
            <button onClick={() => onChange(role, Math.max(0, roles[role] - 1))}
              className="w-7 h-7 rounded-lg border border-stone-200 bg-white text-stone-500 hover:bg-stone-50 transition-colors cursor-pointer text-sm flex-shrink-0">−</button>
            <input type="number" min="0" max="999" value={roles[role]}
              onChange={e => onChange(role, Math.max(0, parseInt(e.target.value) || 0))}
            ...
            <button onClick={() => onChange(role, roles[role] + 1)}
              className="w-7 h-7 rounded-lg border border-stone-200 bg-white text-stone-500 hover:bg-stone-50 transition-colors cursor-pointer text-sm flex-shrink-0">+</button>
          </div>
  ...
  return (
    <div className="bg-white border border-stone-100 rounded-2xl overflow-hidden shadow-sm">
      <div className="bg-[#3D5C3A] px-5 py-4">
        ...
        <h3 style={{fontFamily:'"Cormorant Garamond",serif'}} className="text-[20px] font-semibold text-white leading-tight">{title}</h3>
      </div>
        ...
        {ratioDisclaimer && (
          <div className="mt-4 bg-stone-50 border border-stone-100 rounded-lg px-3 py-2.5">
            <p className="text-[9.5px] text-stone-400 font-light leading-relaxed">
              <span className="font-medium text-stone-500">Role distribution note: </span>
              {ratioDisclaimer}
            </p>
          </div>
        )}
...
function SingleMode() {
  ...
  const type = isCustomType ? null : ARRANGEMENT_TYPES.find(t => t.id === typeId);
    ...
    <div className="max-w-[720px] mx-auto px-4 sm:px-8 py-8">
      ...
      <div className="mb-7">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
          ...
          {/* Custom type */}
          <button onClick={() => setTypeId('custom')}
            className={`text-left px-4 py-3 rounded-xl border transition-all cursor-pointer
              ${isCustomType ? 'bg-[#8B7355] border-[#8B7355] text-white' : 'bg-white border-stone-200 border-dashed hover:border-[#8B7355]/60'}`}>
            <div className="text-lg mb-0.5">✏️</div>
            <div className={`text-[12px] font-medium leading-tight ${isCustomType ? 'text-white' : 'text-stone-600'}`}
              style={{fontFamily:'"Cormorant Garamond",serif'}}>Custom</div>
          </button>
        </div>
      </div>
      ...
      {isCustomType ? (
        ...
        <div className="mb-8">
          <p className="text-[9px] font-medium tracking-[0.18em] uppercase text-stone-400 mb-1">Stems per role</p>
          <p className="text-[11px] text-stone-400 font-light mb-3">Enter exactly how many stems you need for each role.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button onClick={() => setSize('custom')}
                ...
                <div className={`text-[12px] font-medium ${isCustomSize ? 'text-white' : 'text-stone-500'}`}>Custom</div>
                <div className={`text-[10px] ${isCustomSize ? 'text-white/65' : 'text-stone-400'}`}>enter stems</div>
              </button>
            </div>
            {isCustomSize && (
              <div className="mt-3 flex items-center gap-2">
                <label className="text-[11px] text-stone-500 font-light flex-shrink-0">Total stems:</label>
                <input type="number" min="1" max="9999" value={customStems}
                  onChange={e => setCustomStems(Math.max(1, parseInt(e.target.value) || 1))}
          ...
          <div>
            <p className="text-[9px] font-medium tracking-[0.18em] uppercase text-stone-400 mb-3">Arrangement quantity</p>
      ...
      <ResultsCard title={title} counts={counts} cost={cost} ratioDisclaimer={disclaimer}/>
    </div>
  );
}
...
function rowCounts(row) {
  ...
  const stems = row.size === 'custom' ? row.customStems : type.sizes[row.size].stems;
...
function rowLabel(row) {
  ...
  const type = ARRANGEMENT_TYPES.find(t => t.id === row.typeId);
  return row.size === 'custom' ? `${type.name} (${row.customStems} stems)` : `${type.name} — ${type.sizes[row.size].label}`;
}

function EventMode() {
  const [rows, setRows] = useState([defaultRow()]);

  function addRow(custom = false) { setRows(r => [...r, defaultRow(custom)]); }
  ...
  const hasRatioRows = rows.some(r => r.typeId !== 'custom' && r.size === 'custom');

  function copyList() {
    const lines = [
      'STEM COUNT — EVENT ORDER', '',
      ...rows.map(row => `${row.qty}× ${rowLabel(row)} — ${rowCounts(row).total} stems`),
      '',
      'TOTALS BY ROLE',
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col gap-3">
          {rows.map((row, i) => {
            ...
            const type = isCustomType ? null : ARRANGEMENT_TYPES.find(t => t.id === row.typeId);
            const counts = rowCounts(row);

            return (
              <div key={row.id} className={`bg-white border rounded-xl px-4 py-4 ${isCustomType ? 'border-[#8B7355]/25' : 'border-stone-100'}`}>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                  ...
                  {isCustomType ? (
                    <div className="flex gap-2 flex-1 min-w-0">
                      <input type="text" placeholder="Arrangement name…" value={row.customName}
                      ...
                      <button onClick={() => updateRow(row.id, 'typeId', 'hand-tied')}
                    ...
                    <select value={row.typeId}
                      onChange={e => {
                        ...
                        if (val === 'custom') updateRow(row.id, 'size', 'custom');
                      }}
                  ...
                  {!isCustomType && (
                    <select value={row.size} onChange={e => updateRow(row.id, 'size', e.target.value)}
                      ...
                      {type && Object.entries(type.sizes).map(([k, s]) =>
                        <option key={k} value={k}>{s.label} ({s.stems} stems)</option>
                      )}
                  ...
                  <span className="text-[9px] font-medium tracking-[0.12em] uppercase text-stone-400 hidden sm:block">Arrangements</span>
                  <div className="flex items-center gap-2">
                ...
                {isCustomSize && (
                  <div className="mt-3 flex items-center gap-2 sm:pl-8">
                    <label className="text-[11px] text-stone-500 font-light flex-shrink-0">Custom stem count:</label>
                    <input type="number" min="1" max="9999" value={row.customStems}
                      onChange={e => updateRow(row.id, 'customStems', Math.max(1, parseInt(e.target.value) || 1))}
                      className="w-24 px-3 py-1.5 rounded-lg border border-[#8B7355]/50 bg-white text-[13px] font-medium text-stone-800 outline-none focus:border-[#8B7355]"/>
                  </div>
                )}
                ...
                {isCustomType && (
                  <div className="mt-3 sm:pl-8">
                    <p className="text-[9px] font-medium tracking-[0.15em] uppercase text-stone-400 mb-2">Stems per role</p>
                    <RoleInputs roles={row.customRoles} onChange={(role, val) => updateRole(row.id, role, val)}/>
                  </div>
                )}
              </div>
            );
          ...
          <div className="px-5 py-5">
            ...
            {hasRatioRows && (
              <div className="mt-4 bg-stone-50 border border-stone-100 rounded-lg px-3 py-2.5">
                <p className="text-[9.5px] text-stone-400 font-light leading-relaxed">
                  <span className="font-medium text-stone-500">Role distribution note: </span>
                  One or more rows use a custom stem count with a preset arrangement type. Stems are distributed using that type's standard ratios. Use "Custom arrangement" rows to set exact stem counts per role.
                </p>
              </div>
            )}
