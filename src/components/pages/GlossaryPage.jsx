...
function CategoryBadge({ category }) {
  ...
  return (
    <span
      className="text-[9px] font-medium tracking-[0.12em] uppercase px-2 py-0.5 rounded-full border flex-shrink-0"
      style={{
        color: col.text,
        borderColor: col.text + "40",
        background: col.text + "12",
      }}
    >
      {category}
    </span>
    ...
    <div>
      ...
      <div className="bg-[#3D5C3A] px-5 sm:px-14 py-10 sm:py-14 relative overflow-hidden">
        ...
        <p className="text-[10px] tracking-[0.22em] uppercase text-[#B8CEAE] mb-4 font-medium">
          Floral Foundations · Language & Terms
        </p>
        <h1
          style={{ fontFamily: '"Cormorant Garamond",serif' }}
          className="text-[36px] sm:text-[52px] font-semibold text-[#D4B8B5] leading-[1.1] mb-4"
        >
          Floristry <em className="italic">Glossary</em>
        </h1>
        <p className="text-[13px] text-white/55 font-light max-w-lg leading-relaxed">
          {GLOSSARY.length} essential floristry terms — from design principles
          and flower anatomy to mechanics and business vocabulary.
        </p>
      </div>
      ...
      <div className="max-w-[900px] mx-auto px-4 sm:px-10 py-8">
        ...
        <div className="relative mb-6">
          ...
          >
            <path
              d="M6.5 1a5.5 5.5 0 100 11 5.5 5.5 0 000-11zM0 6.5a6.5 6.5 0 1111.65 3.944l2.7 2.7a.5.5 0 11-.707.707l-2.7-2.7A6.5 6.5 0 010 6.5z"
              fill="currentColor"
              fillRule="evenodd"
              clipRule="evenodd"
            />
          ...
          {search && (
            <button
              onClick={() => setSearch("")}
              className="absolute right-3.5 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-600 cursor-pointer bg-transparent border-none text-lg leading-none"
            >
              ×
            </button>
          )}
          ...
          {Object.entries(CATEGORY_COLOURS).map(([cat, col]) => {
            ...
            return (
              ...
              >
                {cat}
                {isActive && (
                  <span className="text-white/80 text-[11px] leading-none font-light ml-0.5">
                    ×
                  </span>
                )}
              </button>
        ...
        {activeCategory && !search && (
          <div
            className="flex items-center justify-between mb-5 px-4 py-3 rounded-xl border"
            style={{
              background:
                (CATEGORY_COLOURS[activeCategory]?.text ?? "#948C82") + "0D",
              borderColor:
                (CATEGORY_COLOURS[activeCategory]?.text ?? "#948C82") + "30",
            }}
          >
            <p
              className="text-[12px] font-medium"
              style={{ color: CATEGORY_COLOURS[activeCategory]?.text }}
            >
              Showing {filtered.length} {activeCategory}{" "}
              ...
              .map(([letter, terms]) => (
                <div key={letter}>
                  <div className="flex items-center gap-3 mb-3">
                    <span
                      style={{ fontFamily: '"Cormorant Garamond",serif' }}
                      className="text-[28px] font-semibold text-[#3D5C3A] leading-none"
                    >
                      {letter}
                    </span>
