  ...
  return (
    <div className="bg-white rounded-xl border border-stone-100 overflow-hidden flex flex-col hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 group">
      <div className="relative overflow-hidden" style={{paddingTop:'65%'}}>
        {loading && <div className="absolute inset-0 animate-pulse bg-stone-100 flex items-center justify-center"><div className="w-5 h-5 rounded-full border-2 border-stone-300 border-t-[#6B8A66] animate-spin"/></div>}
        {!loading && src && <img src={src} alt={flower.name} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"/>}
        {!loading && !src && <div className="absolute inset-0 bg-[#B8CEAE] flex items-center justify-center text-white/70 text-sm font-light">{flower.name}</div>}
        <span className="absolute top-2.5 left-2.5 z-10 text-[9px] font-medium tracking-wide uppercase text-white px-2.5 py-1 rounded-full" style={{background:roleColour}}>{roleLabel}</span>
      </div>
      <div className="p-4 flex flex-col gap-2 flex-1">
        <div>
          <h3 style={{fontFamily:'"Cormorant Garamond",serif'}} className="text-[20px] font-semibold text-stone-800 leading-tight">{flower.name}</h3>
          <p style={{fontFamily:'"Cormorant Garamond",serif'}} className="text-[13px] italic text-[#6B8A66]">{flower.latin}</p>
        </div>
        <p className="text-[12px] text-stone-500 leading-relaxed font-light">{flower.desc}</p>
      </div>
    </div>
  );
}
    ...
    <section className="mb-16">
      <div className="mb-6 pb-4 border-b border-stone-100">
        <div className="flex items-baseline gap-3">
          <span className="text-[9px] font-medium tracking-wide uppercase px-3 py-1 rounded-full text-white" style={{background:roleColour}}>{roleLabel}</span>
          <h2 style={{fontFamily:'"Cormorant Garamond",serif'}} className="text-3xl font-semibold text-stone-800">{title}</h2>
        </div>
