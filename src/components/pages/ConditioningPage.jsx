            ...
            <div key={title} className="bg-white rounded-xl border border-stone-100 overflow-hidden hover:shadow-md transition-all">
              <div className="px-5 py-4 bg-[#3D5C3A]/5 border-b border-stone-100">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <h3 style={{fontFamily:'"Cormorant Garamond",serif'}} className="text-[20px] font-semibold text-stone-800">{title}</h3>
                </div>
                <p className="text-[11px] text-stone-500 font-light tracking-wide">{sub}</p>
              </div>
              <ul className="px-5 py-4 flex flex-col gap-1.5">
                {tips.map((t, i) => <li key={i} className="flex gap-2 text-[12px] text-stone-600 font-light leading-relaxed"><span className="text-[#6B8A66] font-bold flex-shrink-0 mt-0.5">·</span>{t}</li>)}
              </ul>
            </div>
          ))}
        </div>
