...
const HOME_PAGES = [
  {
    ...
    tag: "Reference guide",
    desc: "Photographs, Latin names, UK seasonality, native origins and care notes for the 30 most essential cut flowers and foliages.",
    query: "flower bouquet arrangement",
  },
  {
    page: "wheel",
    label: "Colour Wheel",
    tag: "Interactive tool",
    desc: "An interactive colour wheel covering all five harmony schemes with flower suggestions for each palette.",
    query: "colorful flowers garden",
  },
  {
    page: "bouquets",
    label: "Bouquet Flowers",
    tag: "Bouquet guide",
    desc: "The top 10 focal flowers, top 10 foliage, and top 10 filler flowers for building stunning hand-tied bouquets.",
    query: "hand tied bouquet flowers",
  },
  {
    page: "seasonal",
    label: "Seasonal Calendar",
    tag: "UK seasonal guide",
    desc: "A month-by-month breakdown of what is in season across the UK — from snowdrops in January to dahlias in October.",
    query: "seasonal garden flowers",
  },
  {
    page: "techniques",
    label: "Techniques & Skills",
    tag: "Practical skills",
    desc: "Step-by-step guides to hand-tied bouquets, foam-free mechanics, wiring and taping, wreath making and more.",
    query: "floristry workshop arrangement",
  },
  {
    page: "wedding",
    label: "Wedding Floristry",
    tag: "Event floristry",
    desc: "An introduction to bridal bouquet styles, buttonholes, table centres, ceremony arches, and client consultation.",
    query: "wedding flowers bridal bouquet",
  },
  {
    page: "conditioning",
...
function HomeCard({ p, go }) {
  ...
  return (
    <div
      onClick={() => go(p.page)}
      className="group rounded-2xl overflow-hidden border border-stone-100 bg-white hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col cursor-pointer"
    >
      <div
      ...
      <div className="p-5 flex flex-col gap-2 flex-1">
        <h2
          ...
          className="text-[22px] font-semibold text-stone-800 leading-tight group-hover:text-[#3D5C3A] transition-colors"
        >
          {p.label}
        </h2>
        <p className="text-[12px] text-stone-500 leading-relaxed font-light flex-1">
          {p.desc}
        </p>
        <span className="text-[11px] font-medium text-[#6B8A66] mt-1">
          Explore →
        </span>
      </div>
    </div>
